...
public class UShiftordersUpdate {
    ...
    private final boolean debugMode = "ETN-SRV-MES1".equals(ComputerName);
    ...
    //private final String WP_OP_WORKMODE = "Литье";
    //private final String WP_OP_DEBUGMODE = "%Неразрушающий контроль (РГК)";
    ...
    public void sdiAfterPerformAction(IUserExitParam param) {
        ...
        SpecialParam shiftorder_puffer = aParam.getSpecialParameters().get("shiftorder.puffer");
        ...
        if (shiftorder_status != null && shiftorder_key != null ? shiftorder_key.getValue() != null && !"".equals(shiftorder_status.getValue()) && !"".equals(shiftorder_key.getValue()) : false)
        {
            ...
            try
            {
                ...
                String sql = String.format("Select * From GetShiftOrderData(%s, '%s')", shiftorder_key != null ? shiftorder_key.getValue() : "-1", shiftorder_puffer != null ? shiftorder_puffer.getValue() : "");
                ...
                while (DataSet.next())
                {
                    ...
                    {
                        if (debugPrint) write("UShiftordersUpdate", String.format("auftrag_nr2 = '%s' masch_nr = '%s'", DataSet.getString("auftrag_nr2"), DataSet.getString("masch_nr")));
                ...
                if (debugPrint) write("UShiftordersUpdate", "Update " + affectedRows + " orders");
                    ...
                    if (!ab.isEmpty())
                    {
                        for (String key: ab.keySet())
                        {
                            if (!"".equals(ab.get(key).get("MESOrder")) && !"".equals(ab.get(key).get("Workplace")))
                            {
                                ...
                                {
                                    if (debugPrint) write("UShiftordersUpdate", String.format("RET=%s KT=%s LT=%s ANR %s", retVal.getReturnCode(), retVal.getShortText(), retVal.getLongText(), ab.get(key).get("MESOrder")));
                                }
                                else
                                {
                                    if (debugPrint) write("UShiftordersUpdate", String.format("RET=%s OP %s Finished", retVal.getReturnCode(), ab.get(key).get("MESOrder")));
                                }
                            }
                            ...
                            if (retVal.getReturnCode() != 0) 
                            {
                                if (debugPrint) write("UShiftordersUpdate", String.format("RET=%s KT=%s LT=%s anr %s", retVal.getReturnCode(), retVal.getShortText(), retVal.getLongText(), ab.get(key).get("auftrag_nr")));
                            }
                            else
                            {
                ...
                if ("R".equals(shiftorder_status != null ? shiftorder_status.getValue() : ""))
                {
                    ...
                    if (!ab.isEmpty())
                    {
                        for(String key: ab.keySet())
                        {
                            ...
                            if (retVal.getReturnCode() != 0) 
                            {
                                if (debugPrint) write("UShiftordersUpdate", String.format("RET=%s KT=%s LT=%s ANR %s", retVal.getReturnCode(), retVal.getLongText(), retVal.getShortText(), key));
                            }
                            else
                            {
                                if (debugPrint) write("UShiftordersUpdate", String.format("RET=%s unblock OP %s", retVal.getReturnCode(), key));
                                sql = 
                                " Update " +
                                "   auftrags_bestand " +
                                " Set " +
                                ...
                                if (debugPrint) write("UShiftordersUpdate", " update auftrags_bestand rows count = " + r);
                            }
                            if ("E".equals(rData.get("a_status")))
                            {
                                ...
                                if (retVal.getReturnCode() != 0) 
                                {
                                    if (debugPrint) write("UShiftordersUpdate", String.format("RET=%s KT=%s LT=%s ANR %s", retVal.getReturnCode(), retVal.getShortText(), retVal.getLongText(), key));
                                }
                                else
                                {
                                    if (debugPrint) write("UShiftordersUpdate", String.format("RET=%s Reactivate OP %s", retVal.getReturnCode(), key));
                                }
                            }
                        }
                    }
                    if (!fp.isEmpty())
                    {
                        if (debugPrint) write("UShiftordersUpdate", String.format("fp = [%s]", fp));
                        for (String key: fp.keySet())
                        {
                            ...
                            for (String mnr: fp.get(key))
                            {
                                ...
                                if (debugPrint) write("UShiftordersUpdate", "insert components count = " + i + " anr " + key);
                            }
                        }
                    }
                    if (debugPrint) write("UShiftordersUpdate", up.toString());
                    if (!up.isEmpty())
                    {
                        ...
                        if (debugPrint) write("UShiftordersUpdate", InsertSQL.toString());
                        for (String inssql: InsertSQL)
                        {
                            if (debugPrint) write("UShiftordersUpdate", inssql);
                            int i = stmt.executeUpdate(inssql);
                            if (debugPrint) write("UShiftordersUpdate", "insert rows " + i);
                        }
                    }
                }
                stmt.close();
                ...
                if (debugPrint) write("UShiftordersUpdate", "error = " + e.getMessage());
    ...
    {
        String res =
            " Set IDENTITY_INSERT mlst_hy ON; " +
            " Insert Into " +
            "   mlst_hy " +
            "   ( " +
            "       auftrag_nr, " +
            ...
            "       (Select Max(verweis) + 1 From mlst_hy), " +   //verweis
            ...
            "       NULL, " +                       //kennz_slos
            "       N'N' " +                        //verb_zaehler
            " Set IDENTITY_INSERT mlst_hy OFF;";
        return res;
    }
}