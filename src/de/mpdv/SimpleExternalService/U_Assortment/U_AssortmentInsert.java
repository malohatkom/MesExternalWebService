...
public class U_AssortmentInsert implements ISimpleExternalService 
{
    ...
    public SesResult execute(SesRequest request, SesContext context, ISystemUtilFactory factory) {
        ...
        this.logger = loggerProvider.fetchLogger(U_AssortmentInsert.class);
        this.debugEnabled = this.logger.isDebugEnabled();
        
        if (debugMode) write("U_AssortmentInsert", "U_AssortmentInsert");
        try
        {
            ...
            if (debugMode)
            {
                ...
                {
                    s += " " + key + " = " + (request.getSpecialParam("Assortment." + key) != null ? request.getSpecialParam("Assortment." + key).toString() : "null");
                }
                write("U_AssortmentInsert", s);
            }
            ...
            for (String key: colType.keySet())
            {
                ...
                {
                    if ("AST_WHEEL_KOD".equals(key)) id = request.getSpecialParam("Assortment." + key).getValue() != null ? request.getSpecialParam("Assortment." + key).getValue().toString() : "";
            ...
            if (debugMode) write("U_AssortmentInsert", " fld.length() = " + Integer.toString(fld.length()) + " val.length() " + Integer.toString(val.length()) + " id " + id);
            if (fld.length() != 0 && val.length() != 0 && !"".equals(id)) 
            {
                sql = 
                    ...
                    "           DATEDIFF(SECOND, 0,CONVERT(time, CURRENT_TIMESTAMP)) " + 
                            ...
                            ") " +
                    " End".replace("(, ", "");
                if (debugMode) write("U_AssortmentInsert", sql);
                int i = stmt.executeUpdate(sql);
                if (debugMode) write("U_AssortmentInsert", "rows inserted " + i);
            ...
            Logger.getLogger(U_AssortmentInsert.class.getName()).log(Level.SEVERE, null, ex);
            if (debugMode) write("U_AssortmentInsert", ex.getMessage());
        }        
        finally {
            if (stmt != null) {
                try {
        ...
        try
        {
            if (sParam != null)
            {
                ...
                if (v != null ? !"".equals(v) : false)
                {
                    ...
                    //if (debugMode) write("U_AssortmentInsert", sParam.getAcronym().replace("Assortment.", "") + " : " + t);
