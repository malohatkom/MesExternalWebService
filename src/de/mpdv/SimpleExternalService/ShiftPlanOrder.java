    ...
    public SesResult execute(SesRequest request, SesContext context, ISystemUtilFactory factory)
    {
		...
		ResultSet rs = null;
                ...
                String partSql = "SELECT "
                        ...
                        + "ab.agnr, "
                        + "ab.ag_bez,  "
                        + "hydra1.hydadm.get_datetime(e_anmeld_dat, e_anmeld_zeit) LastLogOn,"
                        + "hydra1.hydadm.get_datetime(prot_dat, prot_zeit) LastLogOff," 
                        + "hydra1.hydadm.get_datetime(ab.term_anf_dat, ab.term_anf_zeit) tss, "
                        + "hydra1.hydadm.get_datetime(ab.term_end_dat, ab.term_end_zeit) tse, "
                        + "hydra1.hydadm.getShift(ab.term_anf_dat, ab.term_anf_zeit, ab.term_end_dat, ab.term_end_zeit) shiftNo, "
                        + "ast.a_status "
                        + ",mas.kostenstelle "
                        + ",ab.kunden_bez"
                        + " FROM hydra1.hydadm.auftrags_bestand ab "
                        + "LEFT OUTER JOIN hydra1.hydadm.auftrag_status ast ON ab.auftrag_nr = ast.auftrag_nr "
                        + "LEFT OUTER JOIN hydra1.hydadm.maschinen mas ON ast.masch_nr=mas.masch_nr "
                        + "LEFT OUTER JOIN hydra1.hydadm.auftrags_bestand oab ON left (ab.auftrag_nr,20)=oab.auftrag_nr "
                        + "LEFT OUTER JOIN hydra1.hydadm.hy_gruppen grp ON ab.mgruppe=grp.gruppe WHERE 1=1"
...
//                    write("C://testJavaCode.txt", strSelStartDate);
                    ...
                    {
                           //" AND mas.bez_lang_18 IS NOT NULL AND ab.agnr IS NOT NULL AND hydra1.hydadm.get_datetime(term_anf_dat, term_anf_zeit) >= '" + strSelStartDate + "'" + " AND hydra1.hydadm.get_datetime(term_end_dat, term_end_zeit) < '" + strSelEndDate + "'" + "order hydra1.hydadm.get_datetime(term_anf_dat, term_anf_zeit) ASC";
                        sql = partSql //+ " AND mas.bez_lang_18 IS NOT NULL AND ab.agnr IS NOT NULL AND hydra1.hydadm.get_datetime(term_anf_dat, term_anf_zeit) >= '" + strSelStartDate + "' AND hydra1.hydadm.get_datetime(term_end_dat, term_end_zeit) < '" + strSelEndDate + "' ORDER BY mas.bez_lang_18, hydra1.hydadm.get_datetime(term_anf_dat, term_anf_zeit) ASC"; 
+ " AND mas.bez_lang_18 IS NOT NULL AND ab.agnr IS NOT NULL AND hydra1.hydadm.get_datetime(ab.term_anf_dat, ab.term_anf_zeit) >= '" + strSelStartDate + "' AND hydra1.hydadm.get_datetime(ab.term_end_dat, ab.term_end_zeit) < '" + strSelEndDate + "' ORDER BY CASE WHEN ab.masch_nr = 'SUPRARX' THEN 1 WHEN ab.masch_nr = 'METALREZ' THEN 2 WHEN ab.masch_nr = 'VAL16-2' THEN 3 WHEN ab.masch_nr = 'VAL16-6' THEN 4 WHEN ab.masch_nr = 'VAL26-15' THEN 5 WHEN ab.masch_nr = 'ROBOT' THEN 6 WHEN ab.masch_nr = 'ROBOTREZ' THEN 7 WHEN ab.masch_nr = 'KOG' THEN 8 WHEN ab.masch_nr = 'PRES4000' THEN 9 WHEN ab.masch_nr = 'PRES7000' THEN 10 WHEN ab.masch_nr = 'SBORKA' THEN 11 WHEN ab.masch_nr = 'CAB_1'  THEN 12 WHEN ab.masch_nr = 'CAB_2'  THEN 13 WHEN ab.masch_nr = 'CAB_3'  THEN 14 WHEN ab.masch_nr = 'CAB_4'  THEN 15 WHEN ab.masch_nr = 'CAB_5' THEN 16 WHEN ab.masch_nr = 'STND_TRN' THEN 17 WHEN ab.masch_nr = 'STND_OTV' THEN 18 WHEN ab.masch_nr = 'STND_D_P' THEN 19 WHEN ab.masch_nr = 'STND_POL' THEN 20 WHEN ab.masch_nr = 'STENDREZ' THEN 21 WHEN ab.masch_nr = 'KALIBR15'  THEN 22 WHEN ab.masch_nr = 'KALIBR30' THEN 23 WHEN ab.masch_nr = 'UZK' THEN 24 WHEN ab.masch_nr = 'REMZONA1'  THEN 25 WHEN ab.masch_nr = 'REMZONA2'  THEN 26 WHEN ab.masch_nr = 'REMZONA3'  THEN 27 WHEN ab.masch_nr = 'REMZONA4' THEN 28 WHEN ab.masch_nr = 'RUCHSVAR' THEN 29 WHEN ab.masch_nr = 'TIP_UP_1'  THEN 30 WHEN ab.masch_nr = 'TIP_UP_2' THEN 31 WHEN ab.masch_nr = 'OTPUSK_1'  THEN 32 WHEN ab.masch_nr = 'OTPUSK_2' THEN 33 WHEN ab.masch_nr = 'MEX_1G_1'  THEN 34 WHEN ab.masch_nr = 'MEX_3G_1'  THEN 35 WHEN ab.masch_nr = 'MEX_3G_2' THEN 36 WHEN ab.masch_nr = 'FINSDACH' THEN 37 WHEN ab.masch_nr = 'UCHUP' THEN 38 ELSE 39 END, hydra1.hydadm.get_datetime(ab.term_anf_dat, ab.term_anf_zeit) ASC";
                    }
                }
                else
                    {
                           sql = partSql + " AND mas.bez_lang_18 IS NOT NULL AND ab.agnr IS NOT NULL ORDER BY mas.bez_lang_18, hydra1.hydadm.get_datetime(ab.term_anf_dat, ab.term_anf_zeit) ASC";  
			...
			+ "WHEN ab.masch_nr = 'METALREZ' THEN 2 "
                        ...
                        + "ELSE 39 END, "
                        + "hydra1.hydadm.get_datetime(ab.term_anf_dat, ab.term_anf_zeit) ASC";
                    }
